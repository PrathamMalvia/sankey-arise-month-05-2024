...
const Profiles = () => {
    ...
    return (
        <section className="relative w-full h-[40rem] overflow-hidden">
            <img src={profileBg} alt="Background" className="w-full h-full object-cover" />
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full lg:w-2/3 flex flex-col lg:flex-row justify-between items-center p-4">
                {profiles.map((profile, index) => (
                    <div key={index} className="relative bg-black bg-opacity-40 rounded-lg p-4 mb-4 lg:mb-0 w-full lg:w-[30%] mx-2">
                        <div className="absolute top-[-25%] left-1/2 transform -translate-x-1/2">
                            <img src={profile.image} alt={profile.name} className="w-24 h-24 rounded-full border-4 border-white" />
                        </div>
                        <div className="mt-12 flex items-center justify-center mb-2">
                            <img src={profile.flag} alt="Flag" className="w-6 h-6 mr-2" />
